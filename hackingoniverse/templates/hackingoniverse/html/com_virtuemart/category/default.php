...
if (count($funcsInfo)) {
    foreach ($funcsInfo as $funcInfo) {
        ...
        $originalPagination = $this->get('vmPagination', null);
        foreach ($this->products as $type => $products) {
            if (count($products) < 1) continue;
            if ($type != 'products') {
                $this->set('vmPagination', null);
            }
            $all = count($products);
            ...
            foreach ($products as $product) {
                ...
                ${'productItemInvisible' . $index} = !$image ? true : false;
                ${'image' . $index} = $image;
            ...
            $this->set('vmPagination', $originalPagination);
        }
    }
}
